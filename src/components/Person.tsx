...
export const Person: React.FC<Props> = ({
  ...
  return (
    <tr data-cy="person" className={location.pathname.includes(slug) ? 'has-background-warning' : undefined}>
      <td>
        <a
      ...
      {nameColor(mother) ? (
        <td>
          <a
            className={nameColor(mother) === 'f' ? 'has-text-danger' : nameColor(mother)}
