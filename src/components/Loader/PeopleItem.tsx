...
export const PeopleItem: React.FC<PersonPageProps> = ({ person }) => {
  ...
  return (
    <tr data-cy="person" className={classNames({ "has-background-warning": selectedPersonId === person.slug })}>
      {/* className="has-background-warning" */}
      <td>
        <NavLink to={`/people/${person.slug}`} className={classNames({ "has-text-danger": person.sex === 'f' })}>
          {person.name}
        </NavLink>
      </td>
